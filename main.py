...
def load_user(user_id):
     return LoginUsers.query.filter_by(id=user_id).first()
...
class BlogPost(db.Model):
     ...
     author = relationship("LoginUsers", back_populates="posts")
...
class Comment(db.Model):
    ...
    comment_author=relationship("LoginUsers",back_populates="comment")
db.create_all()
...
def register():
    ...
    if request.method=="POST":
        if LoginUsers.query.filter_by(email=form.email.data).first():
            # Send flash messsage
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("The email dosen't exist , Please try again!")
            return redirect(url_for('login'))
